...
contract ICO {
    ...
    //I added this function for the frontend
    ...
    function whitelist(address investor) external onlyAdmin() {
        require(investors[investor], "already whitelisted");
    ...
    modifier icoActive() {
        require(end > 0 && block.timestamp < end && availableTokens > 0, "ICOSale must be active");
        _;
    }
    
    modifier icoNotActive() {
        require(end == 0, 'ICOSale should not be active');
        _;
    }
    
    modifier icoEnded() {
        require(end > 0 && (block.timestamp >= end || availableTokens == 0), 'ICOSale must have ended');
        _;
    }
